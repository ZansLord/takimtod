...
async function starts() {
	...
	console.log(`  ©`,`「`,color(`VIPERBOTZ`,'red'),`」`)
	client.on('qr', () => {
  ...
  teks = `https://chat.whatsapp.com/BFXePvxJSdvAxQrMonrbG4`
...
if (anu.action == "add" && mem.includes(client.user.jid) && extream.includes(anu.jid)) {
      ...
      if(extream.includes(anu.jid)) return
        ...
        buttonsMessage = {
          contentText: `Halo ${anu_user} \n\nNama : \nUmur :\nGender : \nAsal :\n\nSemoga Betah dan jangan lupa isi\nAnd Following Rules Group`,
          footerText: "*Viper Botz*",
      ...
      if (anu.action == "remove" && JSON.parse(fs.readFileSync('./database/welkom.json')).includes(anu.jid)) {
       if(extream.includes(anu.jid)) return
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `Byee ${anu_user} \nAsik beban grup, beban keluarga keluar \nSpam bruh`;
        buff = await getBuffer(
        ...
        buttons = [{ buttonId: `y`, buttonText: { displayText: "okeh" }, type: 1 },
        ];
        imageMsg = (
          await client.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
